...
variable "admin_username" {
  description = "Username of the OS. (Defaults can be found here modules/dcos-tested-openstack-oses/azure_template_file.tf)"
  default = ""
}
...
variable "os" {
  default = "coreos"
  description = "Recommended DC/OS OSs are centos_7.2, coreos_1235.9.0, coreos_835.13.0. CoreOS versions are bundled under \"coreos\""
}

variable "master_instance_flavor" {
  description = "OpenStack flavor to use for the DC/OS master instances"
}

variable "bootstrap_instance_flavor" {
  description = "OpenStack flavor to use for the DC/OS Bootstrap instance"
}

variable "private_agent_instance_flavor" {
  description = "OpenStack flavor to use for the DC/OS private agent instances"
}

variable "public_agent_instance_flavor" {
  description = "OpenStack flavor to use for the DC/OS public agent instances"
}
...
variable "ip-detect" {
 type = "string"
 default = "scripts/cloud/openstack/ip-detect.openstack.sh"
...
variable "custom_dcos_download_path" {
 default = ""
 description = "REVIEW: Custom DC/OS version path"
}
...
variable "dcos_resolvers" {
  type = "list"
  description = "DNS Resolver for name resolution. These resolvers are applied both on the public and private subnets as well as used by the tf_dcos_core module."
}
...
variable "dcos_master_discovery" {
 # REVIEW: Should the default be static or master_http_loadbalancer?
 default = "static"
 description = "Ability to use a load balancer or a static list for master discovery - way that agents find master nodes."
}
...
variable "dcos_ip_detect_public_contents" {
 ...
 description = "Used for OpenStack to determine the public IP. Note: single quotes was subsututed for hex x27 as it cannot be used. Currently escapes will need to be performed twice. DC/OS bug requires this variable instead of a file see https://jira.mesosphere.com/browse/DCOS_OSS-905 for more information."
}

variable "os_floating_ip_pool" {
  type = "string"
  description = "OpenStack public floating ip to assign public ips to boot and to master nodes"
}

variable "os_image_name" {
  type = "string"
  description = "Image available in OpenStack to use for VMs"
}

variable "os_external_network_id" {
  type = "string"
  description = "OpenStack network that is to the outside world"
}
