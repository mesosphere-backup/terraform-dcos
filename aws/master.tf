...
resource "null_resource" "dcos-login" {
  depends_on = ["null_resource.master"]

  provisioner "local-exec" {
    working_dir = "scripts/local"
...
resource "null_resource" "install-kubernetes" {
  depends_on = ["null_resource.dcos-login"]

  # Run a local script to install the kubernetes package; this script runs
  # the DC/OS CLI locally to enable the package on the remote DC/OS cluster
  provisioner "local-exec" {
    working_dir = "scripts/local"
    interpreter = ["/bin/bash"]
    command     = "install-kubernetes.sh"
  ...
  provisioner "local-exec" {
    ...
    working_dir = "scripts/local"
    interpreter = ["/bin/bash"]
    command     = "uninstall-kubernetes.sh"
