resource "aws_instance" "private-slave" {
  ...
  connection {
    ...
    bastion_host = "${data.aws_instance.bastion-host.public_ip}"
    agent = true
  }
...
EOF
  ...
  tags {
    Name = "${var.cluster_id}-private-slave-${count.index}"
  }
...
resource "null_resource" "private-slave-provision" {

  count = "${aws_instance.private-slave.count}"

  connection {
    host = "${aws_instance.private-slave.*.public_ip}"
    bastion_host = "${data.aws_instance.bastion-host.public_ip}"
    user = "${var.instance_ami_user}"
  }
